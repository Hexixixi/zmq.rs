...
use coio::sync::mpsc::{channel, Receiver, Sender};
        ...
        match protocol {
            "tcp" => {
                match SocketAddr::from_str(address) {
                    Ok(addr) => {
                        let listener = try!(coio::net::TcpListener::bind(&addr).map_err(ZmqError::from_io_error));
                        TcpListener::spawn_new(listener, self.tx.clone());//, self.options.clone());
                        Ok(())
                    }
